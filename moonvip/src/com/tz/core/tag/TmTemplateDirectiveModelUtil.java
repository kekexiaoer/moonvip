...
//	}
	...
	// 获取Integer类型变量
	public static Integer getInteger(String name, Map<String, TemplateModel> params)
			throws TemplateException {
		TemplateModel model = (TemplateModel) params.get(name);
		if (model == null) {
			return null;
		...
		if (model instanceof TemplateScalarModel) {
			...
			if (TmStringUtils.isBlank(s)){
				return null;
			}
			try {
				return Integer.valueOf(Integer.parseInt(s));
			} catch (NumberFormatException e) {
				if(!TmStringUtils.isNumeric(String.valueOf(s))){
					return null;
				}
		...
		if (model instanceof TemplateNumberModel) {
			return Integer.valueOf(((TemplateNumberModel) model).getAsNumber()
					.intValue());
		}
		throw new TmParameterMustNumberException(name);
	}
	...
	// 获取Integer类型数组
	public static Integer[] getIntegerArray(String name,
			Map<String, TemplateModel> params) throws TemplateException {
		String str = getString(name, params);
		if (TmStringUtils.isBlank(str)) {
			return null;
		}
	...
	public static Long getLong(String name, Map<String, TemplateModel> params)
		...
		if (model instanceof TemplateScalarModel) {
			String s = ((TemplateScalarModel) model).getAsString();
			if (TmStringUtils.isBlank(s) || !TmStringUtils.isNumeric(String.valueOf(s))){
				return null;
			}
			try {
		...
		if (model instanceof TemplateNumberModel) {
			return Long.valueOf(((TemplateNumberModel) model).getAsNumber()
					.longValue());
		}
		throw new TmParameterMustNumberException(name);
	}
	...
	// 获取Boolean型变量
	public static Boolean getBoolean(String name, Map<String, TemplateModel> params)
			throws TemplateException {
		TemplateModel model = (TemplateModel) params.get(name);
		if (model == null) {
			return null;
		...
		if (model instanceof TemplateBooleanModel)
			return Boolean.valueOf(((TemplateBooleanModel) model)
					.getAsBoolean());
		if (model instanceof TemplateNumberModel)
			return Boolean.valueOf(((TemplateNumberModel) model).getAsNumber()
					.intValue() != 0);
		if (model instanceof TemplateScalarModel) {
			String s = ((TemplateScalarModel) model).getAsString();

			if (!(TmStringUtils.isBlank(s))) {
				return Boolean.valueOf((!(s.equals("0")))
