...
@Component("musicPageList")
public class TzMusicListPageTag implements TemplateDirectiveModel ,ServletContextAware {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		tmParams.setPageSize(pageSize==null?30:pageSize);//总条数
		tmParams.setIsDelete(isDelete==null?0:isDelete);//未删除0 1删除
		tmParams.setStatus(status==null?1:status);//默认发布 0未发布1发布
		tmParams.setOrderSql(orderSql==null?" m.hits desc ":orderSql);//排序时间 默认根据创建时间
		List<Music> list = musicDao.findMusicsPage(tmParams);
		if (body != null) {
			int i = 0;
